...
def secant_D(
    ...
    # S / 1.1
    # S * 10**18 / (11 * 10**17)
    D_prev_2: int256 = unsafe_div(unsafe_mul(S, 10**18), 11 * 10**17)

    # gamma**2
    # gamma**2 / 10**18
    gamma2: int256 = unsafe_div(gamma**2, 10**18)

    # (1 + gamma)
    # (10**18 + gamma)
    gamma_1: int256 = unsafe_add(10**18, gamma)

    # ---------------------------------------------------------------------- #
    # Calculate C_D_prev_2:

    # -1.0 / 27 * D**3 * (1.0 + g)**2 # D^9
    # -D * D / 10**18 * D / 10**18 * (10**18 + gamma)**2 / 10**36 / 27
    ...
    d0 = unsafe_div(
        unsafe_mul(
            unsafe_div(d0 * gamma_1, 10**18),
            gamma_1  # <------ safe to do unsafe mul since we did safemul with
        ), #                                      similar numbers just before.
        10**18
    )
    ...
    _d3: int256 = unsafe_sub(
        -81 * 10**18,
        unsafe_mul(54, gamma)
    ) * P
    d3: int256 = unsafe_div(
        unsafe_div(
            unsafe_div(_d3, D_prev_2),
            D_prev_2
         ) * P * 10**18,  # <------------------- Note that we do safemul here.
        D_prev_2
    )
    ...
    d4: int256 = unsafe_div(
        unsafe_div(_d4, D_prev_2) * 10**18,
        D_prev_2
    )
    d4 = unsafe_mul(
        729,
        unsafe_div(
            unsafe_div(d4 * d4, 10**18) * d4,
            10**18
        )
    )

    # Property of (d0, d4) and (d1, d3) pairs: they are somewhat close in
    # magnitude but opposing in sign. This applies to d1 and d3 too. So we
    # can re-arrange their sum and use unsafe math safely.
    ...
    for i in range(255):
        ...
        # unlike before, we can fully do unsafe_mul here, since in linear
        # convergence, D values are closer and we did a similar safemul before.

        d0 = unsafe_div(
            unsafe_div(
                unsafe_mul(
                    unsafe_div(unsafe_mul(-D_prev, D_prev), 10**18),
                    D_prev
                ),
                10**18
            ),
        ...
        d0 = unsafe_div(
            unsafe_mul(
                unsafe_div(unsafe_mul(d0, gamma_1), 10**18),
                gamma_1
            ),
            10**18
        )
        ...
        d3 = unsafe_div(
            unsafe_mul(
                unsafe_div(unsafe_div(_d3, D_prev), D_prev),
                unsafe_mul(P, 10**18)
            ),
            D_prev
        )
        ...
        d4 = unsafe_div(
            unsafe_mul(unsafe_div(_d4, D_prev), 10**18),
            D_prev
        )
        d4 = unsafe_mul(
            729,
        ...
        # Since Secant method is somewhat between quadratic and linear
        # convergence, we expect D_prev and D_prev_2 to be values close to each
        # other. (D_prev - D_prev_2) / inv can entirely be unsafe, safely.
        #
        # Also, C_D_prev * (D_prev - D_prev_2) / inv will be smaller than D_prev
        # and we can hence make the entire expression unsafe, safely.
        D = unsafe_sub(
            D_prev,
            unsafe_div(C_D_prev * unsafe_sub(D_prev, D_prev_2), inv)
        )

        if abs(unsafe_sub(D, D_prev)) < max(100, unsafe_div(D, 10**5)):
