...
def _A_gamma() -> uint256[2]:
    ...
    gamma1: uint256 = A_gamma_1 & 2**128-1  # TODO: check this!
...
def get_xcp(D: uint256) -> uint256:
    ...
    for i in range(1, N_COINS):
        x[i] = D * 10**18 / (N_COINS * (packed_prices & PRICE_MASK))  # ... * PRICE_PRECISION_MUL)
        packed_prices = shift(packed_prices, -PRICE_SIZE)
...
def _claim_admin_fees():
    ...
    D: uint256 = self.get_D(A_gamma[0], A_gamma[1], self.xp())
    self.D = D
...
def tweak_price(A_gamma: uint256[2],
    ...
    for k in range(N_COINS-1):
        price_oracle[k] = packed_prices & PRICE_MASK  # * PRICE_PRECISION_MUL
        packed_prices = shift(packed_prices, -PRICE_SIZE)

    last_prices_timestamp: uint256 = self.last_prices_timestamp
    packed_prices = self.last_prices_packed
    for k in range(N_COINS-1):
        last_prices[k] = packed_prices & PRICE_MASK   # * PRICE_PRECISION_MUL
        packed_prices = shift(packed_prices, -PRICE_SIZE)
    ...
    if new_D == 0:
        ...
        D_unadjusted = self.get_D(A_gamma[0], A_gamma[1], _xp)
    packed_prices = self.price_scale_packed
    for k in range(N_COINS-1):
        price_scale[k] = packed_prices & PRICE_MASK  # * PRICE_PRECISION_MUL
        packed_prices = shift(packed_prices, -PRICE_SIZE)
    ...
    if p_i > 0:
        ...
        __xp[0] += dx_price
        for k in range(N_COINS-1):
            last_prices[k] = price_scale[k] * dx_price / (_xp[k+1] - self.get_y(A_gamma[0], A_gamma[1], __xp, D_unadjusted, k+1))
    ...
    if True:  # scope to reduce size of memory when making internal calls later
        ...
        if True:
            ...
            if t > 0:
                ...
                xp[i] = x0
                self.D = self.get_D(A_gamma[0], A_gamma[1], xp)
                xp[i] = x1  # And restore
        ...
        dy = xp[j] - self.get_y(A_gamma[0], A_gamma[1], xp, self.D, j)
        ...
        if dx > 10**5 and dy > 10**5:
            ...
            if i != 0 and j != 0:
                p = (
                        shift(self.last_prices_packed, -PRICE_SIZE * convert(i-1, int256)) & PRICE_MASK
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    if True:  # Scope to avoid having extra variables in memory later
        ...
        for i in range(1, N_COINS):
            price_scale: uint256 = (packed_prices & PRICE_MASK) * precisions[i]  # * PRICE_PRECISION_MUL
        ...
        t: uint256 = self.future_A_gamma_time
        if t > 0:
            old_D = self.get_D(A_gamma[0], A_gamma[1], xp_old)
            if block.timestamp >= t:
                self.future_A_gamma_time = 1
        else:
            old_D = self.D
...
def _calc_withdraw_one_coin(
    ...
    if True:  # To remove packed_prices from memory
        ...
        xp[0] *= xx[0]
        for k in range(1, N_COINS):
            p: uint256 = packed_prices & PRICE_MASK  # * PRICE_PRECISION_MUL
            if i == k:
    ...
    if update_D:
        D0 = self.get_D(A_gamma[0], A_gamma[1], xp)
    else:
        D0 = self.D
    ...
    y: uint256 = self.get_y(A_gamma[0], A_gamma[1], xp, D, i)
