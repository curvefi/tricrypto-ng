    ...
    if xcp_profit > xcp_profit_a:
        ...
        if fees > 0:
            ...
            if receiver != empty(address):
                ...
                claimed: uint256 = CurveToken(token).mint_relative(
                    receiver, frac
                )
                xcp_profit -= fees * 2
...
def tweak_price(
    ...
    if last_prices_timestamp < block.timestamp:
        ...
        alpha: uint256 = Math(math).halfpow(
    ...
    if needs_adjustment:
        ...
        if norm > adjustment_step**2 and old_virtual_price > 0:
            norm = Math(math).sqrt_int(
                norm / 10**18
            )  # Need to convert to 1e18 units!
